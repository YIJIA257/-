...
void Player::on_input(const ExMessage& msg)
{
	...
	case WM_RBUTTONDOWN:
		...
		BulletTimeManager::instance()->set_status(BulletTimeManager::Status::Entering);//进入子弹时间
		break;

	case WM_RBUTTONUP:
		stop_audio(_T("bullet_time"));
		BulletTimeManager::instance()->set_status(BulletTimeManager::Status::Exiting);//退出子弹时间
		break;
	}
}
...
void Player::on_update(float delta)
{
	...
	//进入攻击状态时，攻击特效位置与角色保持一致
	if (is_attacking)
	{
		current_slash_animation->set_position(get_logic_center());
		current_slash_animation->on_update(delta);//更新动画
	}
...
void Player::on_roll()
{
	...
	velocity.x = is_facing_left ? -SPEED_ROLL : SPEED_ROLL;//根据朝向设置速度
}
